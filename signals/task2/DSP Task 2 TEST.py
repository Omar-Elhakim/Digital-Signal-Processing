...
def AddSignalSamplesAreEqual(
    userFirstSignal, userSecondSignal, Your_indices, Your_samples
):
    ...
    if (len(expected_samples) != len(Your_samples)) and (
        ...
        print(
            "Addition Test case failed, your signal have different length from the expected one"
        )
        return
...
def SubSignalSamplesAreEqual(
    ...
    if (len(expected_samples) != len(Your_samples)) and (
        ...
        print(
            "Subtraction Test case failed, your signal have different length from the expected one"
        )
        return
    for i in range(len(Your_indices)):
        if Your_indices[i] != expected_indices[i]:
...
def NormalizeSignal(MinRange, MaxRange, Your_indices, Your_samples):
    ...
    if (len(expected_samples) != len(Your_samples)) and (
        ...
        print(
            "Normalization Test case failed, your signal have different length from the expected one"
        )
        return
    for i in range(len(Your_indices)):
        if Your_indices[i] != expected_indices[i]:
...
def MultiplySignalByConst(User_Const, Your_indices, Your_samples):
    if User_Const == 5:
        ...
        file_name = (
            ""  # write here path of MultiplySignalByConstant-Signal2 - by 10.txt
        )
